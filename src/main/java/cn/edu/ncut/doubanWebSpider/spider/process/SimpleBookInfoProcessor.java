...
//	public static final String COOKIE = "bid=\"cLFu0ljxKRE\"; gr_user_id=71a8fed9-c730-4e6b-815f-813f99b450f4; ll=\"108288\"; ap=1; viewed=\"3283973_5257905\"; __utmt=1; dbcl2=\"142128977:lKgaClsIDDI\"; ck=\"uNFe\"; gr_session_id_22c937bbd8ebd703f2d8e9445f7dfd03=c78404b1-2e3a-4bb6-afd9-45e6cfdc0d41; __utmt_douban=1; __utma=30149280.1220747617.1453995829.1455611943.1456219113.6; __utmb=30149280.4.10.1456219113; __utmc=30149280; __utmz=30149280.1456219113.6.5.utmcsr=baidu|utmccn=(organic)|utmcmd=organic; __utmv=30149280.14212; push_noty_num=0; push_doumail_num=1";
	...
	public void process(Page page)
	{
		...
		// 当前页面是图书列表页面
		if (page.getUrl().regex("http://www\\.douban\\.com/tag/\\S+/book*").match())
		{
	...
	{
		//获取图书基本信息
		int bookNum = page.getHtml().xpath("//div[@class='mod book-list']/dl").all().size();
		List<String> bookNames = page.getHtml().xpath("//div[@class='mod book-list']/dl/dd/a/text()").all();
		List<String> bookUrls = page.getHtml().xpath("//div[@class='mod book-list']/dl/dd/a/@href").all();
		List<String> bookImgs = page.getHtml().xpath("//div[@class='mod book-list']/dl/dt/a/img/@src").all();
		List<String> bookRatings = page.getHtml().xpath("//div[@class='mod book-list']/dl/dd/div[@class='rating']/span[@class='rating_nums']/text()").all();
		List<String> ddStr = page.getHtml().xpath("//div[@class='mod book-list']/dl").all();
		int index = 0;
		for(String dd : ddStr)
		{
		...
		List<String> otherInfos = page.getHtml().xpath("//div[@class='mod book-list']/dl/dd/div[@class='desc']/text()").all();
		for(int i=0; i<bookNum; i++)
		{
			try
			{
				...
				sbi.setTag(page.getUrl().toString().split("/")[4]);
				// 抽取替他信息
